...
if(count _invs > 0) then
{
	...
	[[0,format["%1 hat Schmuggelware im Wert von $%2 bei sich.",name _civ,[_illegal] call life_fnc_numberText]],"life_fnc_broadcast",west,false] spawn life_fnc_MP;
}
	else
{
	_inv = "Keine illegalen Gegenstände";
};
if(!alive _civ || player distance _civ > 5) exitWith {hint format["%1 konnte nicht durchsucht werden.", name _civ]};
//hint format["%1",_this];
