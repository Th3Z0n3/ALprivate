...
if(_house isKindOf "House_F") then
{
	...
	[[0,format["Ein Haus wird gerade von der Polizei durchsucht!", (_owner select 0)]],"life_fnc_broadcast",true,false] spawn life_fnc_MP;
	_displayName = getText(configFile >> "CfgVehicles" >> (typeOf _house) >> "displayName");
	_upp = format["Knacke Schloss %1",_displayName];
	...
	titleText["Du hast die Haustür geknackt.","PLAIN"];
};