...

{
	...
	{
		_text = format["%1<img size='1.5' image='icons\trendup.paa'/> $%2 <br/>", _text, _globalchange];
	}
	else
	{
		_text = format["%1<img size='1.5' image='icons\trenddown.paa'/> $%2 <br/>", _text, _globalchange];
	};

	if(_lastchange >= 0) then
	{
		_text = format["%1<img size='1.5' image='icons\trendup.paa'/> $%2 <br/>", _text, _lastchange];
	}
	else
	{
		_text = format["%1<img size='1.5' image='icons\trenddown.paa'/> $%2 <br/>", _text, _lastchange];
	};
