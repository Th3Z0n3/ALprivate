...
while {true} do
{
	if((round(_time - time)) > 0) then
	{
		...
		{
			hintSilent format["Verbleibende Zeit:\n %1\n\nDu darfst die Kaution bezahlen\nKaution: $%2",_countDown,[life_bail_amount] call life_fnc_numberText,if(isNil "life_canpay_bail") then {"Ja"} else {"Nein"}];
		}
		else
		{
			hintSilent format["Verbleibende Zeit:\n %1\n",_countDown];
...
switch (true) do
{
	case (_bail) :
	{
		...
		hint "Du hast deine Kaution bezahlt und kannst nun gehen.";
	...
	{
		life_is_arrested = false;
		hint "Du bist aus dem Gefängnis geflohen. Deine vorherigen Delikte wurden nicht gelöscht und wirst zusätzlich wegen der Flucht gesucht.";
		[[0,format["%1 ist aus dem Gefängnis entkommen!",name player]],"life_fnc_broadcast",nil,false] spawn life_fnc_MP;
