...
*/
private["_bad","_unit","_time","_longname"];
...
titleText["Du wurdest festgenommen. Warte bis deine Strafe abläuft. Solltest du versuchen durch einen Respawn oder Disconnect zu entkommen, wird sich deine Strafe erhöhen!","PLAIN"];
hint "Du hast die folgenden Lizenzen aberkannt bekommen\n\nWaffenschein\nRebellenlizenz";
...
if(life_inv_shroomsu > 0) then {[false,"shroomsu",life_inv_shroomsu] call life_fnc_handleInv;};
if(life_inv_shroomsp > 0) then {[false,"shroomsp",life_inv_shroomsp] call life_fnc_handleInv;};
