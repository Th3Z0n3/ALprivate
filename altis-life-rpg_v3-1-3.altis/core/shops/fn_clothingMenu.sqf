...
//Cop / Civ Pre Check
if((_this select 3) in ["bruce","dive","reb"] && playerSide != civilian) exitWith {hint "Du musst Zivilist, sein um diesen Shop zu nutzen!"; closeDialog 0;};
if((_this select 3) == "reb" && !license_civ_rebel) exitWith {hint "Du hast noch kein Rebellentraining!"; closeDialog 0;};
if((_this select 3) in ["cop"] && playerSide != west) exitWith {hint "Du musst Polizist sein, um diesen Shop zu nutzen!"; closeDialog 0;};
...
{
	if(!(missionNamespace getVariable (_var select 0))) exitWith {hint format["Du brauchst ein/eine %1 um in diesen Shop einzukaufen!",[_var select 0] call life_fnc_varToStr]; closeDialog 0;};
};
