...
if(!([_amount] call fnc_isnumber)) exitWith {hint "Du hast keine Zahl eingegeben.";};
_amount = parseNumber (_amount);
if(_amount > (missionNameSpace getVariable _var)) exitWith {hint "Du hast nicht soviele Gegenstände zum Verkaufen!"};

_price = (_price * _amount);
_name = [_var] call life_fnc_vartostr;
if(([false,_type,_amount] call life_fnc_handleInv)) then
{
	hint format["Du hast %1 %2 für $%3 verkauft.",_amount,_name,[round (_price)] call life_fnc_numberText];
	life_cash = life_cash + round (_price);
