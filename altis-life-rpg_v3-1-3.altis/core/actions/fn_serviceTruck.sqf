...
if(count (_nearby) > 1) then
{
	...
	titleText[format["Wartung läuft... %1 Nicht bewegen...",_name],"PLAIN"];
	titleFadeOut 12;
	sleep 10;
	if((vehicle player) distance _vehicle > 10) exitWith {titleText["Du musst in der Nähe bleiben (10m).","PLAIN"];};
