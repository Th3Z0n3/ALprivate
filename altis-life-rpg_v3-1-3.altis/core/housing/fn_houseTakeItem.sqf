...
if(isNull life_trunk_vehicle OR !alive life_trunk_vehicle) exitWith {hint "Der Lagerraum existiert nicht oder wurde zerstört."};
if(!alive player) exitwith {closeDialog 0;};

if((lbCurSel 8502) == -1) exitWith {hint "Du musst einen Gegenstand auswählen!";};
...
if(_num > _value) exitWith {hint "Der Lagerraum hat nicht so viele Gegenstände."};
	...
	else
{
	...
	{
		hint "Aufnehmen gescheitert. Ist dein Inventar voll?";
	};
};