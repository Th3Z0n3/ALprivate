...
if (_house isKindOf "House") then
{
	...
	_type = typeOf _house;
	if (_numDoors < 1) exitWith {hint "Du kannst kein Haus ohne Türen abschliessen!";};
	
	if (_isLocked == 1) then
	{
		_isLocked = 0;
		hint format["Du hast alle Türen aufgeschlossen.", _buildingName];
	}
	else
	{
		_isLocked = 1;
		hint format["Du hast alle Türen abgeschlossen.", _buildingName];
	};
