...
if(_ctrl == "money") then
{
	...
	if(life_cash < _num) exitWith {hint "Du hast nicht genug Geld, um es hier einzulagern!"};
	if(_index == -1) then
	{
...
{
	if(((_totalWeight select 1) + _itemWeight) > (_totalWeight select 0)) exitWith {hint "Der Lagerraum ist voll und kann nicht mehr aufnehmen."};

	if(!([false,_ctrl,_num] call life_fnc_handleInv)) exitWith {hint "Konnte die Gegenstände nicht aus dem Inventar, in den Lagerraum laden.";};
	_index = [_ctrl,_inv] call fnc_index;
	if(_index == -1) then
	{
		_inv set[count _inv,[_ctrl,_num]];
