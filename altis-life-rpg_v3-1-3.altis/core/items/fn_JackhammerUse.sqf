/*
	...
	Original by: Bryan "Tonic" Boardwine
	Modified by Redphoenix
...
switch (true) do
{
	...
	case (player distance (getMarkerPos "oil_1") < 50) : {_mine = "oilu"; _val = 3;};
	case (player distance (getMarkerPos "oil_2") < 50) : {_mine = "oilu"; _val = 3;};
	case (player distance (getMarkerPos "rock_1") < 50): {_mine = "rock"; _val = 3;};
	case (player distance (getMarkerPos "kohle_1") < 50): {_mine = "kohler"; _val = 5;};
	case (player distance (getMarkerPos "silver_1") < 50): {_mine = "silver"; _val = 3;};	
	case (player distance (getMarkerPos "gold_1") < 50): {_mine = "gold"; _val = 3;};
	default {_mine = "";};
};
//Mine check
if(_mine == "") exitWith {hint "Du bist in keiner Mine!"};
if(!license_civ_jackhammer) exitWith {hint "Du kannst den Presslufthammer nicht benutzen. Du bist kein Dimplomingenieur!";};
