...
if(_isVehicle && _curTarget in life_vehicles) exitWith {hint "Du besitzt die Schlüssel zu diesem Fahrzeug bereits."};
...
if((_curTarget isKindOf "LandVehicle") OR (_curTarget isKindOf "Ship") OR (_curTarget isKindOf "Air")) then {
[[_curTarget],"life_fnc_alarmanlage",nil,true] spawn life_fnc_MP;
} else {
titleText["Beeil dich, bevor die Cops dich erwischen.","PLAIN"];
};
...
while {true} do
{
	...
	sleep 0.20; // Dauer des Lockpickings bei Schleifendurchlauf?! Original: 0.26
	_cP = _cP + 0.01;
...
if(!_isVehicle) then {
	...
	if(_dice < 35) then {
		...
		[[0,format["Jemand hat versucht ein Fahrzeug aufzubrechen.",name player]],"life_fnc_broadcast",west,false] spawn life_fnc_MP;
		titleText["Dein Dietrich ist kaputt.","PLAIN"];
	};
};