...
if(_vehicle isKindOf "Air") then {
	...
	sleep 3.35;
	if(local _vehicle) then {
		_vehicle setHit [getText(configFile >> "cfgVehicles" >> typeOf _vehicle >> "HitPoints" >> "HitVRotor" >> "name"), 1];
		sleep (3 * 60);
		_vehicle setHit [getText(configFile >> "cfgVehicles" >> typeOf _vehicle >> "HitPoints" >> "HitVRotor" >> "name"), 0];
	}
};
