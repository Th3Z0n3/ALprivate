...
if((lbCurSel 2005) == -1) exitWith {hint "Du hast keinen Gegenstand ausgewählt.";ctrlShow[2002,true];};
...
if(!([_val] call fnc_isnumber)) exitWith {hint "Keine Zahlen eingegeben.";ctrlShow[2002,true];};
if(parseNumber(_val) <= 0) exitWith {hint "Du musst eine genaue Anzahl angegeben.";ctrlShow[2002,true];};
if(isNil "_unit") exitWith {ctrlShow[2001,true]; hint "Der ausgewählte Spieler ist nicht in Reichweite.";};
if(!([false,_item,(parseNumber _val)] call life_fnc_handleInv)) exitWith {hint "Du hast nicht genügend davon.";ctrlShow[2002,true];};
