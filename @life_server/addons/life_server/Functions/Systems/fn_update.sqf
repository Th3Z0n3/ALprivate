...
//Error check
if(isNull _unit OR _uid == "") exitWith {if(_uid == "" && !isNull _unit) then {diag_log format["%1 had a invalid UID.",_name];};};
...
switch(_side) do
{
	case west: {[[_uid,_name,_side,_money,_bank,_licenses,_misc]] call DB_fnc_addQueue;};
	case civilian: {[[_uid,_name,_side,_money,_bank,_licenses,_misc,_civGear]] call DB_fnc_addQueue;};
	case independent: {[[_uid,_name,_side,_money,_bank,_licenses]] call DB_fnc_addQueue;};
};